...
--
DELIMITER $$
CREATE TRIGGER `Trigger_insert_agilidad_recepcion_ls_2025` AFTER UPDATE ON `icam2025` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `Trigger_insert_indagend_li_2025` AFTER INSERT ON `icam2025` FOR EACH ROW BEGIN
INSERT IGNORE INTO `INDAGEND` (`TRAMITEX`,`TABLAXXX`,`CAMPOXXX`,`REGUSRXX`,`REGFECXX`,`REGHORXX`,`REGFECMX`,`REGHORMX`,`REGESTXX`)
...
--
-- Indices de la tabla `icam2025`
--
ALTER TABLE `icam2025`
